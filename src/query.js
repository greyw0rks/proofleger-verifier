...
switch(cmd) {
  ...
  case "celo": {
    switch(sub) {
      case "stats": { let s; try { s = getCeloStats(db); } catch { console.log("No Celo data yet."); break; } console.log(`\nCelo: ${s.total} txs · ${s.wallets} wallets`); break; }
      ...
      case "wallet": { const addr = rest[0]; if (!addr) break; let rows = []; try { rows = db.prepare("SELECT * FROM celo_proofs WHERE from_address = ?").all(addr); } catch {} console.log(`Celo proofs for ${addr}: ${rows.length}`); break; }
    }
    break;
  }
  case "report": {
  ...
  default:
    console.log("Commands: stats | proof | wallet | recent | search | top | export | celo | report | leaderboard | timeline | anomalies | issuers | nfts | batch");
}
